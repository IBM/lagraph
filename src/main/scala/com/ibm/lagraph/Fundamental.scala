 ...
 * "Fundamental graph algorithms." Graph Algorithms in the Language of Linear Algebra 22 (2011): 45.
 * 
 * TODO: move to tests
 */
object Fundamental {
	 ...
	 */
	def FloydWarshall(sc: SparkContext) = {
			// ********
			...
			val numv = 5L
				...
				List(((x._1._1, x._1._2), x._2),
					...
					type PrimTreeType = Tuple2[Float, Long]
							...
							val diagStripped = rcvGraph.flatMap { kv => if (kv._1._1 == kv._1._2) None else Some((kv._1, (kv._2, kv._1._1))) }
							val rcvAdj = diagStripped.union(sc.range(0L, numv, 1L, rcvGraph.getNumPartitions).map { i => ((i, i), PrimSemiring.zero) })
									// use distributed context-specific utility to convert from RDD to LagMatrix
									...
									//    s_final_test.asInstanceOf[LagDstrVector[PrimTreeType]].dstrBvec.vecRdd.collect.foreach { case (k, v) => println("s_final_test: (%s,%s): %s".format(k._1, k._2, v)) }
									...
									def iterate(weight: Float,
											...
											if (s.equiv(s_final_test)) (weight, d, s, pi) else {
												...
												//        s2.asInstanceOf[LagDstrVector[PrimTreeType]].dstrBvec.vecRdd.collect.foreach { case (k, v) => println("s_i: (%s,%s): %s".format(k._1, k._2, v)) }
												//        val wp = d.ele(u._2)
												...
												val u = hc.vArgmin(hc.vZip(LagSemiring.nop_plus[FloatWithInfType].multiplication, hc.vMap({ wp: PrimTreeType => wp._1 }, d), s))
														val wp = hc.vEle(d, u._2)
														println("  iterate: wp: >%d<".format(u._2))
														iterate(weight + wp._1.get._1, d.zip(PrimSemiring.multiplication, mAdj.vFromRow(u._2)), s.set(u._2, FloatInf), pi.set(u._2, wp._1.get._2))
											}
	 ...
	 * @param sc the Spark context
	 */
	def BellmanFord(sc: SparkContext) = {
			// ********
			// Setup some utility functions: Common
