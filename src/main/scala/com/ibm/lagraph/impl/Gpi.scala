...
// ********
case class GpiBmat[MS: ClassTag](
    val a: GpiAdaptiveVector[GpiAdaptiveVector[MS]]) {
  val n = a.size
  def loadMapBuilder(
    cl: Builder[((Long, Long), MS), Seq[((Long, Long), MS)]],
    rowOffset: Long,
    colOffset: Long): Unit = {
  ...
  override def toString() = a.toString() // added by GABI
}

case class GpiDstrBvec[VS](
    val dstr: GpiDstr,
...
class GpiDstr(
  ...
  require(nblockRequested > 0, "requested number_of_blocks: >%s< must be > 0".format(nblockRequested))
  ...
  //  println("vecClipN: >%s<, vecStride: >%s<, vecClipStride: >%s<".format(vecClipN,vecStride,vecClipStride))
  //  println("remClipN: >%s<, remStride: >%s<, remClipStride: >%s<".format(remClipN,remStride,remClipStride))
  ...
  private def vectorPartitionIndexToKey(index: Int): ((Int, Int)) = {
    ...
    if (rblockin >= clipN) (clipN, (index - clipN * (vecClipN + 1)) % (remClipN + 1)) else (rblockin, index % (vecClipN + 1))
  }
  // ****
  def createVectorBaseRdd(sc: SparkContext) = {
    ...
    sc.parallelize(cpFiltered.map((_, 0)), numSlices = (clipN + 1) * (vecClipN + 1)).partitionBy(new GpiBlockVectorPartitioner(clipN + 1, vecClipN + 1, remClipN + 1)) // 0 is dummy
  }
  ...
  // ****
  def dstrBvecToRvRdd[VS](
    dstrBvec: GpiDstrBvec[VS]): RDD[(Long, VS)] = {
    ...
    def f(input: ((Int, Int), GpiBvec[VS])): List[(Long, VS)] = {
      ...
      if (av.denseCount == 0) {
        ...
        av.denseIterator.toList.map { case (r: Int, v) => (rblock * stridel + cblock * vecstridel + r, v) }
      }
    }
    vecRdd.flatMap(f)
  }
  ...
  // ****
  def dstrBmatAdaptiveToRcvRdd[MS: ClassTag](
    sc: SparkContext,
    dstrBmat: GpiDstrBmat[MS]): RDD[((Long, Long), MS)] = {
    ...
    def perblock(kv: ((Int, Int), GpiBmat[MS])): List[((Long, Long), MS)] = {
      ...
      def rowIterate(
        riter: Iterator[(Int, GpiAdaptiveVector[MS])],
        rcv: List[((Long, Long), MS)]): List[((Long, Long), MS)] = if (!riter.hasNext) rcv else {
        def colIterate(
          cr: Int,
          citer: Iterator[(Int, MS)],
          rcv: List[((Long, Long), MS)]): List[((Long, Long), MS)] = if (!citer.hasNext) rcv else {
          val (cc, cv) = citer.next()
          colIterate(cr, citer, ((rblockl * stridel + cr.toLong, cblockl * stridel + cc.toLong), cv) :: rcv)
        }
  ...
  def dstrBmatAdaptiveFromRcvRdd[MS: ClassTag](
    ...
    val full_map = rr.map {
      case ((i, j), elem) => {
        val (rgin, cgin, rlin, clin) = ((i / chunk).toInt, (j / chunk).toInt, (i % chunk).toInt, (j % chunk).toInt)
        val (rg, rl) = if (rgin > clipN) (clipN, rlin + (rgin - clipN) * chunk) else (rgin, rlin)
        val (cg, cl) = if (cgin > clipN) (clipN, clin + (cgin - clipN) * chunk) else (cgin, clin)
        ((rg, cg, rl, cl), elem)
      }
    } // .cache()
    ...
    implicit val csrOrdering = new Ordering[(Int, Int, Int, Int)] {
      override def compare(l: (Int, Int, Int, Int), r: (Int, Int, Int, Int)): Int = {
        ...
        val rr = r._3.toLong
        val rc = r._4.toLong
        val lthis = if (l._2 == clipN) lr * csrChunkClipL + lc else lr * csrChunkL + lc
        val rthat = if (r._2 == clipN) rr * csrChunkClipL + rc else rr * csrChunkL + rc
        if (lthis < rthat) -1 else {
    ...
    //    full_map_partitioned.collect().foreach { case (k, v) => println("full_map_partitioned: k: (%s, %s, %s, %s) v: %s".format(k._1,k._2,k._3,k._4,v)) }
      ...
      iter: Iterator[((Int, Int, Int, Int), MS)]): Iterator[((Int, Int), GpiBmat[MS])] =
      {
        ...
        if (iter.hasNext) {
          ...
          while (iter.hasNext) {
            ...
            val v = ircv._2
            if (lr != lrs) {
              val rseq = GpiAdaptiveVector.fromRvSeq(crBuffer, cvBuffer, msSparse, cChunk)
              rBuffer(lrs) = rseq
              crBuffer.clear()
              cvBuffer.clear()
              lrs = lr
    ...
    // **
    val mRdd_withNones = full_map_partitioned.mapPartitionsWithIndex(addArrays, true).cache()
    mRdd_withNones.setName("mRdd_withNones")
    //    mRdd_withNones.collect().foreach { case (k, v) => println("mRdd_withNones: (%s,%s): %s".format(k._1, k._2, GpiSparseRowMatrix.toString(v.asInstanceOf[GpiBmatAdaptive[MS]].a))) }
  ...
  def dstrBvecFromMap[VS: ClassTag](
    sc: SparkContext,
    v: Map[Long, VS],
    vsSparse: VS,
  ...
  // ****
  private def dstrBvecFromCons[VS: ClassTag](
    sc: SparkContext,
    vsCons: VS,
    vsSparse: VS): GpiDstrBvec[VS] = {
    ...
    val aRdd_cons = createVectorBaseRdd(sc).map(consBlockerV) //.partitionBy(new GpiBlockVectorPartitioner(clipN + 1, vecClipN + 1, remClipN + 1))
  ...
  private def dstrBvecFromRvRdd[VS: ClassTag](
    sc: SparkContext,
    ...
    val bvecSparseVecClipStride = GpiBvec(GpiAdaptiveVector.fillWithSparse[VS](vecClipStride)(vsSparse))
    val bvecSparseRemStride = GpiBvec(GpiAdaptiveVector.fillWithSparse[VS](remStride)(vsSparse))
    val bvecSparseRemClipStride = GpiBvec(GpiAdaptiveVector.fillWithSparse[VS](remClipStride)(vsSparse))
    ...
    def partitionVectorIntoBlocks(rv: (Long, VS)): ((Int, Int), (Int, VS)) = {
      ...
      val (rblock, cabs) = if (rblockin > this.clipN) (this.clipN.toLong, rabs % stridel + (rblockin - this.clipN) * stridel) else (rblockin, rabs % stridel)
      if (rblock == this.clipN) {
        val cblockin = cabs / remStridel
        val (cblock, rlocal) = if (cblockin > this.clipN) (this.clipN.toLong, cabs % remStridel + (cblockin - this.clipN) * remStridel) else (cblockin, cabs % remStridel)
        //        println("rblock: >%s<, cabs: >%s<, cblockin: >%s<, cblock: >%s<, rlocal: >%s<, value: >%s<".format(rblock,cabs,cblockin,cblock,rlocal,rv._2))
        ((rblock.toInt, cblock.toInt), (rlocal.toInt, rv._2))
      } else {
        val cblockin = cabs / vecStridel
        val (cblock, rlocal) = if (cblockin > this.clipN) (this.clipN.toLong, cabs % vecStridel + (cblockin - this.clipN) * vecStridel) else (cblockin, cabs % vecStridel)
        ((rblock.toInt, cblock.toInt), (rlocal.toInt, rv._2))
      }
    }
    //    println(aRdd_init.collect.size)
    //    aRdd_init.collect().foreach { case (k, v) => println("aRdd_init: k: %s, v: %s".format(k,v)) }
    val vRdd_tagged = aRdd_init.map(partitionVectorIntoBlocks)

    //    vRdd_tagged.collect().foreach { case (k, v) => println("vRdd_tagged: k: (%s, %s) v: (%s, %s)".format(k._1,k._2,v._1,v._2)) }
    val vRdd_partitioned = vRdd_tagged.combineByKey(List[(Int, VS)](_), (x: List[(Int, VS)], y: (Int, VS)) => x :+ y, (x: List[(Int, VS)], y: List[(Int, VS)]) => x ::: y)

    //    vRdd_partitioned.collect().foreach { case (k, v) => println("vRdd_partitioned: k: (%s, %s) v: %s".format(k._1,k._2,v)) }
    ...
    def toBvec(kv: ((Int, Int), List[(Int, VS)])): ((Int, Int), GpiBvec[VS]) = {
      ...
      //      println("rblock: >%s<, cblock: >%s<, curStride: >%s<".format(rblock, cblock, curStride))
    ...
    //    aRdd_bvec.collect().foreach { case (k, v) => println("aRdd_bvec: k: (%s, %s) v: %s".format(k._1,k._2,v.u.toVector)) }
    def addSparseBvec(index: Int, iter: Iterator[((Int, Int), GpiBvec[VS])]): Iterator[((Int, Int), GpiBvec[VS])] = {
    ...
    val aRdd_complete = aRdd_bvec.partitionBy(new GpiBlockVectorPartitioner(clipN + 1, vecClipN + 1, remClipN + 1)).mapPartitionsWithIndex(addSparseBvec, true)

    val aRdd = if (cache) aRdd_complete else aRdd_complete.cache()
    aRdd.setName("aRdd")
    //    aRdd.collect().foreach { case (k, v) => println("aRdd: k: (%s, %s) v: %s".format(k._1,k._2,v.u.toVector)) }
    GpiDstrBvec(this, aRdd, vsSparse)

  }

  // ****
  private def dstrBvecFromIndices(
    sc: SparkContext,
    offset: Long,
    ...
    def indexBlockerV(kv: ((Int, Int), Int)): ((Int, Int), GpiBvec[Long]) = {
      ...
      //      println("clipN >%s<, stride >%s<, clipStride >%s<".format(clipN, stride, clipStride))
      //      println("vstride >%s<, vclipstride >%s<, rblock: >%s<, cblock: >%s<, stride: >%s<, clipstride: >%s<, curVectorStride: >%s<, begin: >%s<, end: >%s<".format(
    ...
    val aRdd_indices = createVectorBaseRdd(sc).map(indexBlockerV) //.partitionBy(new GpiBlockVectorPartitioner(clipN + 1, vecClipN + 1, remClipN + 1))
   ...
   */
  def dstr_replicate[T: ClassTag](sc: SparkContext, x: T, sparseValueOpt: Option[T] = None): GpiDstrBvec[T] = {
   ...
   */
  def dstr_indices(sc: SparkContext, start: Long, sparseValueOpt: Option[Long] = None): GpiDstrBvec[Long] = {
   ...
   */
  def dstr_reduce[T1, T2: ClassTag](f: (T1, T2) => T2, c: (T2, T2) => T2, z: T2, u: GpiDstrBvec[T1]): T2 = {
   ...
   *
   */
  def dstr_map[T1: ClassTag, T2: ClassTag](f: (T1) => T2, u: GpiDstrBvec[T1], sparseValueT2Opt: Option[T2] = None): GpiDstrBvec[T2] = {
    ...
    val sparseValueT2 = sparseValueT2Opt.getOrElse(f(u.sparseValue))
    val mappedDVV = u.vecRdd.mapValues(fv).partitionBy(new GpiBlockVectorPartitioner(clipN + 1, vecClipN + 1, remClipN + 1))
   ...
   */
  def dstr_zip[T1: ClassTag, T2: ClassTag, T3: ClassTag](f: (T1, T2) => T3, u: GpiDstrBvec[T1], v: GpiDstrBvec[T2], sparseValueT3Opt: Option[T3] = None): GpiDstrBvec[T3] = {
    ...
    def fco(input: (Iterable[GpiBvec[T1]], Iterable[GpiBvec[T2]])): GpiBvec[T3] = {
      ...
      //x      val wa = LagUtils.timeblock(GpiOps.gpi_zip(f, ua, va, sparseValueT3Opt, Option(ua.threshold)), "gpi_zip")
      val wa = GpiOps.gpi_zip(f, ua, va, sparseValueT3Opt, Option(ua.threshold))
      //      // SPARSE
      //      if (wa.denseCount == 0) (indx, GpiBvec(GpiAdaptiveVector.fillWithSparse[T3](stride)(sparseValueT3))) else (indx, GpiBvec(wa))
      GpiBvec(wa)
    }
   ...
   *  @param f semiring addition (commutative monoid with identity element) used for merging values within a partition
   ...
   *
   */
  def dstr_innerp[T1: ClassTag, T2: ClassTag, T3: ClassTag, T4: ClassTag](f: (T3, T4) => T4, c: (T4, T4) => T4, g: (T1, T2) => T3, zero: T4, u: GpiDstrBvec[T1], v: GpiDstrBvec[T2], sparseValueT3Opt: Option[T3] = None): T4 = {
...
object GpiDstr {
  ...
  def debugSummaryOfDstrBmatSparsity[MS](db: GpiDstrBmat[MS]): String = {
    ...
    //      db.matRdd.collect().map{ case (k, v) => "(%d,%d)".format(k._1,k._2)}.mkString("[", ",", "]")
  }
  ...
  def countDstrBvecNonzeros(dstrBvec: GpiDstrBvec[_]): Long = {
    ...
    val checkTotalNonzeros = dstrBvec.vecRdd.mapValues(countTotalNonzeros).values.reduce(reduceCountTotalNonzeros)
    checkTotalNonzeros
  }
}
...
class GpiBlockVectorPartitioner(rblocks: Int, cblocks: Int, remCblocks: Int) extends Partitioner {
  ...
  override def getPartition(key: Any): Int = key match {
    case (i: Int, j: Int) => {
      if (i == rblocksm1) (i * cblocks) + j % remCblocks else (i * cblocks) + j % cblocks
    }
    case _ =>
