...
sealed protected trait AdaptiveVectorToBuffer {
  ...
  def toDenseBuffer[VS: ClassTag](rv: (GpiBuffer[Int], GpiBuffer[VS]), sparseValue: VS, size: Int): GpiBuffer[VS] = {
  ...
  def toSparseBuffers[VS: ClassTag](iseq: GpiBuffer[VS], sparseValue: VS, denseCount: Int): (GpiBuffer[Int], GpiBuffer[VS]) = {
    val t0 = System.nanoTime()
    val res = GpiBuffer.rvDenseBufferToSparseBuffer(iseq, denseCount, sparseValue)
    val t1 = System.nanoTime()
    val t01 = LagUtils.tt(t0, t1)
...
// https://github.com/twitter/algebird/blob/develop/algebird-core/src/main/scala/com/twitter/algebird/AdaptiveVector.scala
sealed trait GpiAdaptiveVector[VS] {
  def length = size
  def sparseValue: VS
  def denseCount: Int // number of items that are not sparse
  def size: Int
  def threshold: Double
  def apply(i: Int): VS
  def updated(i: Int, v: VS): GpiAdaptiveVector[VS] // return new AV w/ updated value
  def extend(count: Int, value: VS): GpiAdaptiveVector[VS] // Grow by adding count sparse values to the end
...
object GpiAdaptiveVector extends AdaptiveVectorToBuffer with Serializable {
  ...
  def fillWithSparse[VS: ClassTag](
    size: Int,
    threshold: Double = GpiAdaptiveVector.DefaultThreshold)(sparseValue: VS): GpiAdaptiveVector[VS] = GpiSparseVector[VS]((GpiBuffer.empty[Int], GpiBuffer.empty[VS]), sparseValue, size, threshold)
  def fromSeq[VS: ClassTag](v: Seq[VS],
                            sparseValue: VS,
    ...
    if (v.size == 0) {
      ...
      if (denseCount < sz * threshold)
        GpiSparseVector[VS](toSparseBuffers(GpiBuffer(v.toArray), sparseValue, denseCount), sparseValue, sz, threshold)
      else
  ...
  def wrapDenseArray[VS: ClassTag](a: Array[VS],
  ...
  def fromRvSeq[VS: ClassTag](
    ...
    if (rseq.size == 0) {
      ...
      if (denseCount < sizeOfDense * threshold)
        GpiSparseVector[VS](GpiBuffer.rvSeqToSparseBuffers(rseq, vseq, sparseValue, denseCount), sparseValue, sizeOfDense, threshold)
      else
        GpiDenseVector[VS](GpiBuffer.rvSeqToDenseBuffer(rseq, vseq, sparseValue, sizeOfDense), sparseValue, denseCount, threshold)
    }
  }
  def fromMap[VS: ClassTag](
    ...
    if (m.size == 0) {
      ...
      if (denseCount < sizeOfDense * threshold) {
        ...
        GpiSparseVector[VS](GpiBuffer.rvSeqToSparseBuffers(rseq, cseq, sparseValue, denseCount), sparseValue, sizeOfDense, threshold)
      } else {
        val (rseq, cseq) = m.toSeq.unzip
        GpiDenseVector[VS](GpiBuffer.rvSeqToDenseBuffer(rseq, cseq, sparseValue, sizeOfDense), sparseValue, denseCount, threshold)
      }
    }
  }
  ...
  def concatenateToDense[VS: ClassTag](vs: Seq[GpiAdaptiveVector[VS]]): GpiAdaptiveVector[VS] = {
    def concat(sz:Int, i:Int, va:Seq[GpiAdaptiveVector[VS]], vf:Array[VS]): Array[VS] = i match {
      case `sz` => vf
      case _ => concat(sz, i+1, va, vf ++ GpiAdaptiveVector.asDenseBuffer(va(i)).toArray)
    }
  ...
  def toDense[VS: ClassTag](v: GpiAdaptiveVector[VS]): GpiAdaptiveVector[VS] = {
    if (v.isInstanceOf[GpiSparseVector[VS]]) {
      val iSeq = GpiBuffer.rvSparseBuffersToDenseBuffer(v.asInstanceOf[GpiSparseVector[VS]].rv,v.sparseValue,v.size)
  ...
  def gpi_reduce[@spec(Int) VS: ClassTag, @spec(Int) T2: ClassTag](
    ...
    u match {
      case uSparse: GpiSparseVector[VS] => {
        ...
        val (pt1, ops1) = uSparse.rv._2.foldLeft(zero) { case (x, y) => f(x, y) }
        val res = if (f(u.sparseValue, zero) == zero)
          (pt1, ops1)
        else {
          var pt2 = pt1
    ...
    val res =
      (u, v) match {
        case (uSparse: GpiSparseVector[VS], vSparse: GpiSparseVector[VS]) => {
          GpiBuffer.gpiCompareSparseSparse(
            uSparse.rv,
            vSparse.rv,
            uSparse.length,
            uSparse.sparseValue,
        ...
        case (uSparse: GpiSparseVector[VS], vDense: GpiDenseVector[VS]) => {
          GpiBuffer.gpiCompareSparseDense(
            uSparse.rv,
            vDense.iseq,
            uSparse.length,
            uSparse.sparseValue,
        ...
        case (uDense: GpiDenseVector[VS], vSparse: GpiSparseVector[VS]) => {
          GpiBuffer.gpiCompareDenseSparse(
            uDense.iseq,
            vSparse.rv,
            uDense.length,
            uDense.sparseValue,
        ...
        case (uDense: GpiDenseVector[VS], vDense: GpiDenseVector[VS]) => {
          GpiBuffer.gpiCompareDenseDense(
            uDense.iseq,
            vDense.iseq,
            uDense.length,
            uDense.sparseValue,
  ...
  def gpi_map[@spec(Int) VS: ClassTag, @spec(Int) T2: ClassTag](
    ...
    val ((bC, denseCountC, ops), utype) =
      u match {
        case uSparse: GpiSparseVector[VS] => {
          (GpiBuffer.gpiMapSparseBuffersToDenseBuffer(
            uSparse.rv,
            uSparse.length,
            uSparse.sparseValue,
            sparseValue,
        ...
        case uDense: GpiDenseVector[VS] => {
          (GpiBuffer.gpiMapDenseBufferToDenseBuffer(
            uDense.iseq,
            sparseValue,
            f),
            "dense")
    ...
    val res = if (denseCountC < u.length * threshold) {
      //x      println("AdaptiveVector: gpi_map: start denseToSparse: VS: >%s<, T2: >%s<".format(xVS, xT2))
      GpiSparseVector[T2](GpiAdaptiveVector.toSparseBuffers(bC, sparseValue, denseCountC), sparseValue, u.length, threshold)
    } else {
      //x      println("AdaptiveVector: gpi_map: start denseToDense: VS: >%s<, T2: >%s<".format(xVS, xT2))
      GpiDenseVector[T2](bC, sparseValue, denseCountC, threshold)
    }
    val t1 = System.nanoTime()
    val t01 = LagUtils.tt(t0, t1)
    //x    println("AdaptiveVector: gpi_map: stop: utype: >%s<, time: >%.3f< s".format(utype, t01))
    res
  }
...
//    println("WHAT!", u, v)
    require(u.length == v.length,
      "gpi_zip: attempt to zip two unequal length vectors, u.length: >%d<, v.length: >%d<".format(
        u.length, v.length))
    val ftype = f match {
      case sg: LagSemigroup[_] => {
        // TODO TODO need to fix this!
        if (sg.annihilator.isEmpty) "notspecified" else {
          val aga = sg.annihilator.get
          if (aga == u.sparseValue && aga == v.sparseValue) {
            if (false) println("gpi_zip: multiplication:  >%s< >%s< >%s<".format(u.sparseValue, v.sparseValue, aga));
            "multiplication"
          } else {
            if (false) println("gpi_zip: notspecified:  >%s< >%s< >%s<".format(u.sparseValue, v.sparseValue, aga));
            "notspecified"
          }
        }
      }
    ...
    ftype match {
        ...
        (u, v) match {
          ...
          case (uDense: GpiDenseVector[VS], vDense: GpiDenseVector[T2]) => {
            ...
            if (denseCountC < uDense.length * threshold) {
              GpiSparseVector[T3](GpiAdaptiveVector.toSparseBuffers(vC, sparseValue, denseCountC), sparseValue, uDense.length, threshold)
            } else {
      ...
      case "notspecified" => {
        ...
        val res = if (denseCountC < vC.length * threshold) {
          //x          println("AdaptiveVector: gpi_zip: start denseToSparse: VS: >%s<, T2: >%s<, T3: >%s<".format(xVS, xT2, xT3))
          GpiSparseVector[T3](GpiAdaptiveVector.toSparseBuffers(vC, sparseValue, denseCountC), sparseValue, vC.length, threshold)
        } else {
          //x          println("AdaptiveVector: gpi_zip: start sparseToDense: VS: >%s<, T2: >%s<, T3: >%s<".format(xVS, xT2, xT3))
          GpiDenseVector[T3](vC, sparseValue, denseCountC, threshold)
        }
        val t1 = System.nanoTime()
        val t01 = LagUtils.tt(t0, t1)
  ...
  def gpi_zip_with_index_special[@spec(Int) VS: ClassTag, @spec(Int) T3: ClassTag](
    ...
    thresholdOpt: Option[Double] = None,
    stats: Option[GpiAdaptiveVector.Stat] = None): GpiAdaptiveVector[T3] = {
    val sparseValue = zeroTargetOpt.getOrElse(f(u(0), 0L)) // no target zero specified, just grab one
    val threshold = if (thresholdOpt.isDefined) thresholdOpt.get else u.threshold
    val xVS = classTag[VS]
    val xT3 = classTag[T3]
    u match {
      case uSparse: GpiSparseVector[VS] => {
        if (zeroSourceOpt.isDefined && zeroSourceOpt.get == uSparse.sparseValue) {
          ...
          val (vC, denseCountC, ops) =
            GpiBuffer.gpiZipSparseWithIndexToDense(
              uSparse.rv,
              uSparse.length,
              base,
              uSparse.sparseValue,
          ...
          if (denseCountC < uSparse.length * threshold) {
            GpiSparseVector[T3](GpiAdaptiveVector.toSparseBuffers(vC, sparseValue, denseCountC), sparseValue, uSparse.length, threshold)
          } else {
            GpiDenseVector[T3](vC, sparseValue, denseCountC, threshold)
          }
        }
      ...
      case uDense: GpiDenseVector[VS] => {
        val (vC, denseCountC, ops) =
          ...
          GpiBuffer.gpiZipDenseWithIndexToDense(
            uDense.iseq,
            uDense.length,
            base,
            uDense.sparseValue,
...
//        }
  ...
  def gpi_zip_with_index_matrix[@spec(Int) VS: ClassTag, @spec(Int) T3: ClassTag](
    ...
    val sparseValue = zeroTargetOpt.getOrElse(f(u(0), (0L, 0L))) // no target zero specified, just grab one
    val threshold = if (thresholdOpt.isDefined) thresholdOpt.get else u.threshold
    val xVS = classTag[VS]
    val xT3 = classTag[T3]
    u match {
...
//              f)
//          if (stats.isDefined) stats.get.increment(f, ops)
//          // cover case where sparse becomes dense, eg due to insertion of diagonal element TODO unit test
//          if (denseCountC < uSparse.length * threshold) {
//            GpiSparseVector[T3](rv, sparseValue, u.length, threshold)
//          } else {
//            GpiDenseVector[T3](GpiAdaptiveVector.toDenseBuffer(rv, sparseValue, u.length), sparseValue, denseCountC, threshold)
//          }
//        } else {
          //x            println("AdaptiveVector: gpi_zip_with_index_matrix: zero not sparse_, uSparse, base")
          val (vC, denseCountC, ops) =
            GpiBuffer.gpiZipSparseWithIndexToDenseMatrix(
              uSparse.rv,
              uSparse.length,
              rowIndex,
              base,
...
//        }
      ...
      case uDense: GpiDenseVector[VS] => {
        val (vC, denseCountC, ops) =
          ...
          GpiBuffer.gpiZipDenseWithIndexToDenseMatrix(
            uDense.iseq,
            uDense.length,
            rowIndex,
            base,
  ...
  class Stat {
    ...
    // http://stackoverflow.com/questions/2657940/how-can-i-take-any-function-as-input-for-my-scala-wrapper-method
    // increment
    def increment(f: => Any, value: Long): Unit = {
      f match {
        case sg: LagSemigroup[_] => {
          if (sg.annihilator.isEmpty) incrementAdd(value) else incrementMul(value)
        }
        case _ => {
...
final case class GpiDenseVector[VS: ClassTag](
  iseq: GpiBuffer[VS],
  ...
  override def apply(idx: Int) = iseq(idx)
  override def updated(idx: Int, v: VS): GpiAdaptiveVector[VS] = {
    ...
    if (denseCount < size * threshold) {
      GpiSparseVector[VS](GpiAdaptiveVector.toSparseBuffers(newIseq, sparseValue, newDenseCount), sparseValue, size, threshold)
    } else {
  ...
  override def extend(cnt: Int, value: VS): GpiAdaptiveVector[VS] = {
    ...
    if (value == sparseValue)
      if (denseCount < newSize * threshold) {
        GpiSparseVector[VS](GpiAdaptiveVector.toSparseBuffers(iseq, sparseValue, denseCount), sparseValue, newSize, threshold)
      } else {
  ...
  override def denseIterator =
    iseq.toList.view.zipWithIndex.filter { _._1 != sparseValue }.map { _.swap }.iterator
...
final case class GpiSparseVector[VS: ClassTag](
    rv: (GpiBuffer[Int], GpiBuffer[VS]),
    override val sparseValue: VS,
    override val size: Int,
    override val threshold: Double) extends GpiAdaptiveVector[VS] {
  ...
  override def updated(indx: Int, v: VS): GpiAdaptiveVector[VS] = {
    ...
    if (v == sparseValue) {
      ...
      if (r == None) {
        val newRv = GpiBuffer.rvInsertItemInSparseBuffers(rv, cursor.get, indx, v)
        val sv = GpiSparseVector(newRv, sparseValue, size, threshold)
        if (denseCount + 1 < size * threshold)
          sv
        else
          GpiDenseVector[VS](GpiAdaptiveVector.toDenseBuffer(sv.rv, sparseValue, size), sparseValue, denseCount + 1, threshold)
      } else {
  ...
  override def extend(delta: Int, value: VS): GpiAdaptiveVector[VS] = {
    ...
    if (value == sparseValue)
      ...
      val (newRv, densecount) = GpiBuffer.rvExtendSparseBuffersWithNonSparseValue(rv, size, delta, value)
      if (denseCount < newSize * threshold)
        GpiSparseVector(newRv, sparseValue, newSize, threshold)
      else
        GpiDenseVector[VS](GpiAdaptiveVector.toDenseBuffer(newRv, sparseValue, newSize), sparseValue, denseCount, threshold)
    }
  }
  // TODO potential performance issue
  override def toVector: Vector[VS] = {
  ...
  override def denseIterator = sortedList.iterator
...
object GpiSparseRowMatrix extends Serializable {
  def toString[T](m: GpiAdaptiveVector[GpiAdaptiveVector[T]]) = {
  ...
  def fromVector[T: ClassTag](vMatrix: Seq[Seq[T]], sparseValue: T): GpiAdaptiveVector[GpiAdaptiveVector[T]] = {
    val mm = MMap[Int, GpiAdaptiveVector[T]]()
    val nrow = vMatrix.size
    val ncol = vMatrix(0).size
    vMatrix.zipWithIndex.foreach { case (v, r) => mm(r) = GpiAdaptiveVector.fromSeq(v, sparseValue) }
  ...
  def toVector[T: ClassTag](aa: GpiAdaptiveVector[GpiAdaptiveVector[T]]): Vector[Vector[T]] = {
    ...
    aa match {
      case gaa: GpiAdaptiveVector[GpiAdaptiveVector[T]] => {
        ...
        rv.foreach { r => cl += GpiAdaptiveVector.toVector(r) }
        cl.result.toVector
      }
    }
  }
  // http://stackoverflow.com/questions/8415812/collection-mutable-openhashmap-vs-collection-mutable-hashmap
  def fromMap[T: ClassTag](
    mMatrix: Iterable[((Int, Int), T)],
    sparseValue: T,
    nrow: Int,
    ...
    //    (0 until nrow) map { i => rohm(i) = new OpenHashMap[Int, T](10) } // TODO not 10 put some powerlaw calc
    (0 until nrow) map { i => rohm(i) = MMap[Int, T]() }
    ...
    rohm map { case (r, m) => sohm(r) = GpiAdaptiveVector.fromMap(m, sparseValue, ncol) }
    val em = GpiAdaptiveVector.fillWithSparse[T](ncol)(sparseValue)
    GpiAdaptiveVector.fromMap(sohm, em, nrow)
  }

  def fromMapOfMap[T: ClassTag](
    mMatrix: Iterable[(Int, Iterable[(Int, T)])],
    sparseValue: T,
    nrow: Int,
    ncol: Int): GpiAdaptiveVector[GpiAdaptiveVector[T]] = {
    val mm = MMap[Int, GpiAdaptiveVector[T]]()
    mMatrix.map { case (k, v) => mm(k) = GpiAdaptiveVector.fromMap(v, sparseValue, ncol) }
    val em = GpiAdaptiveVector.fillWithSparse[T](ncol)(sparseValue)
    GpiAdaptiveVector.fromMap(mm.toMap, em, nrow)
  }
  ...
  def toMap[T: ClassTag](aa: GpiAdaptiveVector[GpiAdaptiveVector[T]]): Map[(Int, Int), T] = {
    ...
    aa match {
      case gaa: GpiAdaptiveVector[GpiAdaptiveVector[T]] => {
        ...
        mas.map { case (r, rm) => GpiAdaptiveVector.toMap(rm).map { case (c, v) => mm((r, c)) = v } }
        mm.toMap
      }
  ...
  def toMapOfMap[T: ClassTag](aa: GpiAdaptiveVector[GpiAdaptiveVector[T]]): Map[Int, Map[Int, T]] = {
    val mm = MMap[Int, Map[Int, T]]()
    aa match {
      case gaa: GpiAdaptiveVector[GpiAdaptiveVector[T]] => {
        GpiAdaptiveVector.toMap(gaa).map { case (r, v) => mm(r) = GpiAdaptiveVector.toMap(v) }
        //          val mas = GpiAdaptiveVector.toMap(gaa)
  ...
  def transpose[T: ClassTag](aa: GpiAdaptiveVector[GpiAdaptiveVector[T]]): GpiAdaptiveVector[GpiAdaptiveVector[T]] = {
  ...
  def slice[T: ClassTag](
    aa: GpiAdaptiveVector[GpiAdaptiveVector[T]],
    rowRange: (Int, Int),
