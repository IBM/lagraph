 ...
 */
final case class LagSmpContext(override val graphSize: Long)
    extends LagContext {
  ...
  override def vFromSeq[T: ClassTag](
    v: Seq[T],
    sparseValue: T): LagVector[T] = {
  ...
  override def mFromMap[T: ClassTag](mMap: Map[(Long, Long), T], sparseValue: T): LagMatrix[T] = {
    ...
    LagSmpMatrix(this, mMap, GpiSparseRowMatrix.fromMap(mtoint, sparseValue, nrow.toInt, ncol.toInt))
  }
    ...
    case ma: LagSmpMatrix[_] => {
      ...
      aa match {
        case gaa: GpiAdaptiveVector[GpiAdaptiveVector[T]] => {
          ...
          (GpiSparseRowMatrix.toMap(gaa).map { case (k, v) => ((k._1.toLong, k._2.toLong) -> v) }, gaa(0).sparseValue)
        }
  ...
  private[lagraph] override def mMap[T1: ClassTag, T2: ClassTag](
    ...
    case ma: LagSmpMatrix[_] => {
      ...
      mFromMap(ma.rcvMap.map { case (k, v) => (k, f(v)) }.filter { case (k, v) => v != sparseValue }, sparseValue)
    }
  }
  private[lagraph] override def mZip[T1: ClassTag, T2: ClassTag, T3: ClassTag](
    ...
    case (ma: LagSmpMatrix[_], na: LagSmpMatrix[_]) => {
      ...
      val mapZip = GpiSparseRowMatrix.toMap(vovZip).map { case (k, v) => ((k._1.toLong, k._2.toLong) -> v) }
  ...
  private[lagraph] override def mSparseZipWithIndex[T1: ClassTag, T2: ClassTag](
    ...
    case ma: LagSmpMatrix[T1] => {
      val rcvMap = if (visitDiagonalsOpt.isDefined) {
        val diagSeq = (0L until ma.size._1).map { d => ((d, d), visitDiagonalsOpt.get) }
        (ma.rcvMap.toSeq.flatMap { case (k, v) => if (k._1 != k._2) Some((k, v)) else None } ++ diagSeq).toMap
      } else ma.rcvMap
  ...
  private[lagraph] override def mTm[T: ClassTag](
    ...
    case (ma: LagSmpMatrix[T], na: LagSmpMatrix[T]) => {
      require(ma.vov(0).sparseValue == na.vov(0).sparseValue, "mTm does not currently support disparate sparsities")
      val msSparse = ma.vov(0).sparseValue
      val vovMtM = GpiOps.gpi_m_times_m(sr.addition, sr.multiplication, sr.addition, sr.zero, ma.vov, na.transpose.asInstanceOf[LagSmpMatrix[T]].vov, Option(msSparse), Option(msSparse))
      val mapMtM = GpiSparseRowMatrix.toMap(vovMtM).map { case (k, v) => ((k._1.toLong, k._2.toLong) -> v) }
  ...
  private[lagraph] def mOp[T: ClassTag](
    ...
    case (ma: LagSmpMatrix[T], na: LagSmpMatrix[T]) => {
      ...
      val mac = ma.rcvMap.toSeq.filter { case (k, v) => k._2 == mCol }.map { case (k, v) => Tuple2(k._1, v) }
      val nar = na.rcvMap.toSeq.filter { case (k, v) => k._1 == nRow }.map { case (k, v) => Tuple2(k._2, v) }
      val cp = for (r <- mac; c <- nar) yield (r, c)
      val rcvMap = cp.map { case (p1, p2) => Tuple2(Tuple2(p1._1, p2._1), f.multiplication.op(p1._2, p2._2)) }.toMap
  ...
  private[lagraph] def vFromMcol[T: ClassTag](
    ...
    case ma: LagSmpMatrix[_] =>
      vFromMap(ma.rcvMap.filter { case (k, v) => k._2 == mCol }.map { case (k, v) => (k._1, v) }, ma.vov(0).sparseValue)
  }
