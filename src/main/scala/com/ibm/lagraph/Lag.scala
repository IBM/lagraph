...
case class LagSemigroup[T](
    ...
    identity: Option[T] = None) extends Function2[T, T, T] {
...
object LagSemigroup {
  ...
  val infRegistry = Map[ClassTag[_], Any](
    classTag[Float] -> Float.MaxValue,
    classTag[Double] -> Double.MaxValue,
    classTag[Int] -> Int.MaxValue,
    classTag[Long] -> Long.MaxValue)
  def infinity[T](tt: ClassTag[T]): T = LagSemigroup.infRegistry(tt).asInstanceOf[T]
  // a registry of minus infinity values for basic scala types
  val minfRegistry = Map[ClassTag[_], Any](
    classTag[Float] -> Float.MinValue,
    classTag[Double] -> Double.MinValue,
    classTag[Int] -> Int.MinValue,
    classTag[Long] -> Long.MinValue)
  def minfinity[T](tt: ClassTag[T]): T = LagSemigroup.minfRegistry(tt).asInstanceOf[T]
}
...
trait LagSemiringAsNumeric[T] extends Numeric[T] {
  ...
  def toDouble(x: T): Double = throw new RuntimeException("toDouble not required")
}
/**
...
object LagSemiring {
  ...
  def apply[T: ClassTag](
    addition: Function2[T, T, T],
    multiplication: Function2[T, T, T],
    zero: T,
    one: T): LagSemiring[T] = {
    val additionSg = LagSemigroup(addition, identity = Option(zero))
    val multiplicationSg = LagSemigroup(multiplication, annihilator = Option(zero), identity = Option(one))
    //    new LagSemiring(additionSg, multiplicationSg)
    new LagSemiring[T] { protected def tt = reflect.classTag[T]; val addition = additionSg; val multiplication = multiplicationSg }
  }
  /**
   * The plus.times semiring. If the underlying type is not a scala primative type (e.g., Float, Double, Int, or Long,
   *  then user must define an implicit object that extends LagSemiringAsNumeric[T].
   *
  ...
  /**
   * The or.and semiring. If the underlying type is not a scala primative type (e.g., Float, Double, Int, or Long,
   *  then user must define an implicit object that extends LagSemiringAsNumeric[T].
   *
   *  @tparam T the underlying type.
   *
  ...
  def or_and: LagSemiring[Boolean] = {
    ...
    val additionSg = LagSemigroup[Boolean](
      addition,
      identity = Option(zero))
    val multiplicationSg = LagSemigroup[Boolean](
      multiplication,
      annihilator = Option(zero),
      identity = Option(one))
 ...
 */
abstract class LagVector[T: ClassTag] protected (val hc: LagContext, val size: Long) extends Serializable {
  ...
  def map[T2: ClassTag](
    ...
    hc.vMap(f, this)
  /**
   * Return a vector representing the application of a binary operator to the pairs of elements formed by zipping together
   ...
   *  @return resultant vector
   */
  def zip[T2: ClassTag, T3: ClassTag](
    f: (T, T2) => T3,
    v: LagVector[T2]): LagVector[T3] =
    hc.vZip(f, this, v)
  /**
   * Return a vector representing the application of a binary operator to pairs of elements formed by zipping together
   *  this vector with an (implicit) index vector
   *
   *  @tparam T2 the element type of the returned vector
   *
   *  @param f the function to be applied to each pair of elements
   ...
   *  @return resultant vector
   */
  def zipWithIndex[T2: ClassTag](
    f: (T, Long) => T2,
    sparseValueOpt: Option[T2] = None): LagVector[T2] =
    hc.vZipWithIndex(f, this, sparseValueOpt)
  /**
   * Using functions f and c and an starting value along w/ an (implicit) index vector, reduce this vector to scalar.
   *  This function is similar to 'vZip' except the reduction function takes a pair elements formed by
   ...
   *  @return the result of the reduction
   */
  def reduceWithIndex[T2: ClassTag](
    f: ((T, Long), T2) => T2,
    c: (T2, T2) => T2, // TODO need help from eknath
    z: T2): T2 =
 ...
 */
abstract class LagMatrix[T: ClassTag] protected (val hc: LagContext, val size: (Long, Long)) extends Serializable {
/**
 ...
 */
   ...
   * Transform a matrix.
   *
   *  The primary purpose of this function is to transform a canonical adjacency matrix into
   *  an adjacency matrix suitable for a particular LAG algorithm.
   *  All non-sparse entries and (optionally) the diagonal are transformed using an input function.
   *  Sparse values in the input matrix are directly transformed into 'targetSparseValue'
   *  If visitDiagonalsOpt is not specified or specified as None then diagonal elements are only
   *  transformed if they are non-sparse.
   *
   *  @tparam T2 the type of the result
   *
   *  @param f binary operator that performs the transforms an element of the input matrix and its indices
   *           into an element at the same location in the output matrix
   *  @param targetSparseValue sparse elements in the input matrix are transformed into elements with this value
   *  @param visitDiagonalsOpt if not 'None", diaganol elements are transformed using 'visitDiagonalsOpt.get' as input to 'f'
   *
   *  @return a matrix resulting from the transformation
   */
  @deprecated("Not in scope", "0.1.0")
  def sparseZipWithIndex[T2: ClassTag](
    f: (T, (Long, Long)) => T2,
...
trait LagContext {
   ...
   *  @param f the function to be applied to each element
   *  @param u the input vector
   *  @return a new vector resulting from the application of the function to each element in the input vector
   */
  private[lagraph] def vMap[T1: ClassTag, T2: ClassTag](
    f: (T1) => T2,
    u: LagVector[T1]): LagVector[T2]
  /**
   * Builds a new vector by applying a binary operator to pairs of elements formed by zipping together
   *  two input vectors
   *
  ...
  /**
   * Builds a new vector by applying a binary operator to pairs of elements formed by zipping together
   *  the input vector with an index vector
   *
   *  @tparam T1 the element type of the first input vector
   *  @tparam T2 the element type of the returned vector
   *
  ...
  private[lagraph] def vZipWithIndex[T1: ClassTag, T2: ClassTag](
    ...
    sparseValueOpt: Option[T2] = None): LagVector[T2]
  /**
   * Using functions f and c and an starting value along w/ an internal index vector, reduce a vector to scalar.
   *  This function is similar to 'vZip' except the reduction function takes a pair elements formed by
   *  zipping togehter the input vector with an index vector.
   *
   *  @tparam T1 the type of the elements in the input vector
  ...
  // *****
  // experimental
  private[lagraph] def mMap[T1: ClassTag, T2: ClassTag](
    f: (T1) => T2,
  ...
  private[lagraph] def mZipWithIndex[T1: ClassTag, T2: ClassTag](
    f: (T1, (Long, Long)) => T2,
    m: LagMatrix[T1]): LagMatrix[T2]
  ...
  // experimental
  private[lagraph] def mTm[T: ClassTag](
    sr: LagSemiring[T],
    m: LagMatrix[T],
    n: LagMatrix[T]): LagMatrix[T]
  // Hadagard .multiplication
   ...
   */
  private[lagraph] def mTranspose[T: ClassTag](
    m: LagMatrix[T]): LagMatrix[T]
  /**
   ...
   *  @return true if l is equivalent to r; false otherwise.
   */
  private[lagraph] def vEquiv[T: ClassTag](
    l: LagVector[T], r: LagVector[T]): Boolean
  // ********
  /**
   ...
   */
  private[lagraph] def vEle[T: ClassTag](u: LagVector[T],
                                         r: Long): (Option[T], Long) = {
    val f = (tl: (T, Long), tr: (Option[T], Long)) =>
      if (tr._2 == r) (tr._1, r) else if (tl._2 == r) (Option(tl._1), r) else (None, -1L)
   ...
   */
  private[lagraph] def vSet[T: ClassTag](u: LagVector[T],
                                         r: Long,
                                         v: T) = {
  ...
  private[lagraph] def vArgmin[T: ClassTag](v: LagVector[T])(implicit n: Numeric[T]): (T, Long) = {
    val f = (tl: (T, Long), tr: (Option[T], Long)) => {
      ...
      else if (n.lt(tl._1, tr._1.get)) (Option(tl._1), tl._2) else tr
    }
    val c = (tl: (Option[T], Long), tr: (Option[T], Long)) => {
      if (tr._1.isEmpty) tl
      else if (tl._1.isEmpty) tr
      else if (n.lt(tl._1.get, tr._1.get)) tl else tr
    }
...
object LagContext {
   ...
   */
  def getLagSmpContext(graphSize: Long): LagSmpContext = LagSmpContext(graphSize)
  /**
   ...
   */
  def getLagDstrContext(sc: SparkContext, graphSize: Long, nblock: Int) =
  ...
  def vectorOfVectorToString[T](a: Vector[Vector[T]], tToStr: (T) => String): String = {
    val as = a.map { r => { r.map { v => tToStr(v) } } }
   ...
   */
  def vectorOfVectorFromMap[T](m: Map[(Long, Long), T], sparseValue: T, size: (Long, Long)): Vector[Vector[T]] = {
    val nrow = size._1
    val ncol = size._2
  ...
  def mapFromSeqOfSeq[T: ClassTag](vMatrix: Seq[Seq[T]], sparseValue: T): Map[(Long, Long), T] = {
    ...
    vMatrix.zipWithIndex.map { case (rv, r) => rv.zipWithIndex.filter { case (v, c) => v != sparseValue }.map { case (v, c) => mo((r.toLong, c.toLong)) = v } }
    mo.toMap
  }
}
