...
class LagSemiringSuite extends FunSuite with Matchers {
  ...
  def srTests[T](sr: LagSemiring[T],
    ...
    if (DEBUG) {
      println("1) check that (T, addition, zero) is positive monoid w/ identity zero")
    }
    if (DEBUG) {
      println("1a) associative: a addition ( b addition c) = (a addition b) addition c")
    }
    for (a <- ts) {
      if (valid(a)) {
        for (b <- ts) {
    ...
    if (DEBUG) {
      println("2) (S, multiplication, one) is a monoid w/ identity one")
    }
    if (DEBUG) {
      println(
    ...
    if (DEBUG) {
      println("2b) identity: a multiplication one = one multiplication a = a")
    }
    for (a <- ts)
      if (valid(a)) {
  ...
  test("testStandards") {
    def test[T: ClassTag](testList: List[T])(implicit num: Numeric[T]): Unit = {
      val tt = classTag[T]
    ...
    val intList = List(-199, 99, 0, 99, 199)
    test[Int](intList)
    if (DEBUG) println("Long")
    val longList = List(-199L, 99L, 0L, 99L, 199L)
    test[Long](longList)
    if (DEBUG) println("Float")
    val floatList = List((-199).toFloat, (99).toFloat, (0).toFloat, (99).toFloat, (199).toFloat)
    test[Float](floatList)
    if (DEBUG) println("Double")
    val doubleList =
      List((-199).toDouble, (99).toDouble, (0).toDouble, (99).toDouble, (199).toDouble)
...
//      //      checkh2 = if (w == BfCons.wZero) (h == BfCons.hopZero) && (n == BfCons.nodeNil)
//      //        else true
...
//      //      if (!passed) println("Rejected: >%s<".format(Tuple3(wstr, hstr, nstr))) else
//      //        println("Passed: >%s<".format(Tuple3(wstr, hstr, nstr)))
//      passed
//    }
